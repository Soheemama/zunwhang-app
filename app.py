...
# 2. ★ 마마님의 비밀 지도 (코스닥/코스피 정밀 교정 완료) ★
# K-반도체(445380)만 .KS, 나머지는 마마님 말씀대로 .KQ로 전수 수정했습니다.
portfolio_map = {
    ...
    "AI반도체소부장 (SOL)": {"ticker": "475370.KQ", "price": 19330.0, "cur": "₩"},
    "전고체배터리 (SOL)": {"ticker": "465540.KQ", "price": 16968.0, "cur": "₩"},
    "코리아휴머노이드 (TIGER)": {"ticker": "475380.KQ", "price": 13026.0, "cur": "₩"},
    "현대차그룹플러스 (TIGER)": {"ticker": "415480.KS", "price": 55794.0, "cur": "₩"},
    "코스닥150 (KODEX)": {"ticker": "159400.KQ", "price": 19540.0, "cur": "₩"},
    "조선 TOP3 (SOL)": {"ticker": "466920.KQ", "price": 38282.0, "cur": "₩"}
}
...
if symbol:
    ...
    def get_data(ticker):
        try:
            # interval="1d"와 progress=False로 안정성을 높였습니다.
    ...
    if data is not None and not data.empty:
        ...
        st.error(f"⚠️ 데이터 로드 실패. '{symbol}' 티커가 시장(KOSPI/KOSDAQ)에 맞는지 확인 중입니다.")
