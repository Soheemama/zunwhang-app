...
def load_data_ultra(ticker):
    # 1. 야후 정규 규격 시도
    df = yf.download(ticker, period="1y", interval="1d", progress=False)
    if not df.empty: return df, "Primary"
    
    # 2. 야후 교차 규격 시도 (.KS <-> .KQ)
    alt = ticker.replace(".KS", ".KQ") if ".KS" in ticker else ticker.replace(".KQ", ".KS")
    df = yf.download(alt, period="1y", interval="1d", progress=False)
    if not df.empty: return df, "Secondary"
    
    # 3. 야후 번호 전용 규격 시도
    clean_ticker = ticker.split(".")[0]
    df = yf.download(clean_ticker, period="1y", interval="1d", progress=False)
    if not df.empty: return df, "Direct"
    
    return None, None

data, source = load_data_ultra(info['y'])
