...
# ★ [긴급] 네이버 데이터 직송 보급로 (Scraping Engine) ★
@st.cache_data(ttl=60)
def load_data_emergency(item):
    if item['cur'] == "₩":
        try:
            url = f"https://fchart.naver.com/sise.nhn?symbol={item['n']}&timeframe=day&count=300&requestType=0"
            r = requests.get(url, timeout=5)
            data = []
            for line in r.text.strip().split('\n'):
                if '<item data=' in line:
                    v = line.split('"')[1].split('|')
                    data.append([v[0], float(v[1]), float(v[2]), float(v[3]), float(v[4]), int(v[5])])
    ...
    # 미국 종목 혹은 네이버 실패 시 야후 보급로 (2중 백업)
    try:
        df = yf.download(item['y'], period="1y", interval="1d", progress=False)
        if not df.empty: return df, "야후 보급"
    except: pass
        
    return None, None

data, source = load_data_emergency(info)
...
if data is not None and not data.empty:
    ...
    with col1:
        ...
        else: st.info(f"📍 [관망] 고점 대비 안정적 전황 유지")
    ...
    st.error(f"⚠️ 현재 모든 보급로가 일시적으로 차단되었습니다. 잠시 후 새로고침(F5) 해주세요.")
